...
def main():
    ...
    data = queryImage.queryImage()
    newData = np.frombuffer(data,dtype=np.uint8)
    # bgr_image = cv2.medianBlur(newData, 3)
    image = cv2.imdecode(newData, 1)
    ...
    edges = cv2.Canny(img_gray, 600, 601)
    cv2.imshow('edges', edges)
    # This returns an array of r and theta values
    lines = cv2.HoughLinesP(edges, 1, np.pi / 180, 100, minLineLength=300, maxLineGap=350)
    ...
    # Show result
    img = cv2.resize(image, dsize=(600, 600))
